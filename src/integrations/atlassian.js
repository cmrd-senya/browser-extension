...
myObserver = new MutationObserver((mutations) => {
    ...
    const mapedNodes = mutations.map(function (mutation) {
        return document.evaluate( `//*[text()='${selectedIssue}']`, mutation.target, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue
    }).filter(Boolean);
    if (!mapedNodes.length) {
        return;
    }
    renderToNode(mapedNodes[0])
});
